...
export default function AdminChat() {
  const navigation = router; // Expo Router v2 uyumluluğu için router kullanıldı
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Sadece normal kullanıcıları getir (admin olmayanları)
      const q = query(usersRef, where('userType', '==', 'user'));
      ...
      for (const doc of querySnapshot.docs) {
        ...
        let name = data.name || email.split('@')[0] || 'Kullanıcı';
  ...
  const fetchMessages = () => {
    if (!selectedUser) return; // Sadece seçili kullanıcı varsa mesajları getir

    const messagesRef = collection(db, 'messages');
    // Sadece bireysel mesajları getir (grup mesajı olmayanları)
    const q = query(
      messagesRef,
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Okunmamış mesajları okundu olarak işaretle (adminin aldığı mesajlar)
  ...
  const sendMessage = async () => {
    if (!message.trim() || !selectedUser) return; // Sadece mesaj ve seçili kullanıcı varsa gönder
    ...
    try {
      const messageData: {
        ...
        isRead: false,
        senderName: 'Yönetici', // Admin olarak gönderiliyor
        senderEmail: auth.currentUser?.email,
        isGroupMessage: false, // Bireysel mesaj
      };

      // Admin mesaj gönderdiği için daire bilgileri eklemeye gerek yok
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    const isAdmin = item.senderId === auth.currentUser?.uid;
    const sender = users.find(user => user.id === item.senderId);

    let senderName = isAdmin ? 'Yönetici' : sender?.name || item.senderName || item.senderEmail?.split('@')[0] || 'Kullanıcı';

    // Yönetici mesajlarında daire bilgisi göstermeye gerek yok
    if (!isAdmin) {
       const block = sender?.block || '';
       const apartmentNo = sender?.apartmentNo || '';
       if (block && apartmentNo) {
         senderName = `${senderName} (${block} Blok ${apartmentNo})`;
       }
    }
  ...
  return (
    <View style={styles.container}>
      <Image source={logoImage} style={[styles.backgroundImage, { opacity: 0.05 }]} resizeMode="cover" />
      ...
      <LinearGradient
        ...
        <Text style={styles.headerTitle}>
          {selectedUser ? users.find(user => user.id === selectedUser)?.name || 'Kullanıcı' : 'Kullanıcı Mesajları'}
        </Text>
